...
public class WuAndPalmer
{
	...
	public double wupExtented(String w1, int s1, String w2, int s2, String pos, double extension1, double extension2)
	{
		double 			wup 		= 0.0;
		IIndexWord	word1	=	null;
		IIndexWord 	word2	=	null;
		...
		else // [Case 2.] find the wup score (as per Perl ... mad!!!)
		{
			...
			for(ArrayList<ISynsetID> p1 : paths1)
			{
				for(ArrayList<ISynsetID> p2 : paths2)
				{
					double score = lookingExtented(p1, p2, extension1, extension2);
					if(score > maxscore)
					{
						maxscore = score;
					}
	...
	private double lookingExtented(ArrayList<ISynsetID> p1, ArrayList<ISynsetID> p2, double extension1, double extension2)
	{
		...
		/*
		 * fg: correzione perchè nella formula manca (2.0 * lcs) e estensione
		 */
		score = (2.0 * lcs) / (d1 + (extension1) + d2 + extension2 + (2.0 * lcs));
